...
export default function ProductView() {
  ...
  return (
    <div className='m-auto w-[calc(100%-200px)] md:w-[768px] '>
      ...
      <p className='text-center'>當獸醫師為您的毛孩評估後，建議適合且特定的營養需求，您可安心使用希爾思處方食品，為毛孩的每一天提供最好的照護。</p>
      <div className='card py-[15px] px-[20px]'>
        <div className='text-lg font-bold mb-6'>希爾思處方食品提供經臨床實證的科學營養，滿足每個寵物的獨特營養需求。</div>
        <div className='grid grid-cols-2 gap-4'>
          {demands.map((demand) => {
            return (<div className='flex content-center gap-4' key={demand.name}>
              <img className='w-[60px]' src={demand.img} />
              <p>{demand.name}</p>
            </div>);
          })}
        </div>
      ...
      <div className='card py-[15px] px-[20px] text-center'>
        ...
        <div className='mb-6'> 我們的處方食品提供專業的寵物營養，需經過獸醫師評估及指示後使用。請點選以下連結，協助您就近找到獸醫師，為您評估寵物的健康。</div>
        <Divider></Divider>
        <div className='text-center'>
          <Button className='my-6' variant='contained' onClick={findAnimalHospital}>尋找動物醫院</Button>
        </div>
      </div>
